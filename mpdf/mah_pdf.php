...
function showDocInfo($doctorId, $yAxis, $size){
	...
	if($resultData['prescriptionStyle'] == 2){
		...
		$this->MultiCell(100,5, "ডাঃ আলী হোসেন", 0);
		...
		$this->MultiCell(100,5, "এমবিবিএস, এফসিপিএস (মেডিসিন), এমডি (বক্ষব্যাধি)", 0);
		$this->SetXY(120, $yAxis);
		...
		$this->SetXY(15, $yAxis);
		$this->SetFont('nikosh','',$size + 2);
		$this->MultiCell(100,5, "মেডিসিন ও বক্ষব্যাধি বিশেষজ্ঞ", 0);
		$this->SetXY(120, $yAxis);
		$this->SetFont('nikosh','',$size);
		...
		$this->SetXY(10, 255);
		$this->SetFont('nikosh','',$size);
		$this->MultiCell(100,5, "প্রেসক্রিপশনে লিখা ঔষধের অন্য নাম গ্রহনযোগ্য নয়।", 0);
