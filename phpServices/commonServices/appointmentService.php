...
function getAppointment($username, $date){
	...
	JOIN appointment_type at ON at.id= app.appointmentType
	LEFT JOIN patient p ON app.patientCode = p.patientCode
	WHERE app.doctorCode = '$username' AND app.date='$date' order by app.appointmentID DESC";	
	$result=mysql_query($sql);
...
function getNextDateAppointment($username, $filteredDate){
	$sql = "	
		...
		WHERE  app.doctorCode = '$username'   and app.appointmentType >0     AND n.date >= '$filteredDate' 
...
function PatientAddd($username, $date){
	$sql = "SELECT
	...
	left join diagnosis d on d.appointMentID=app.appointmentID
	left join disease ds on d.diseaseID = ds.id	
	WHERE app.doctorCode = '$username' AND app.patientCode=$date order by app.appointmentID DESC";
...
function getPatientByDisease($username, $disease_search_str){
	$sql = "		
			...
			WHERE app.doctorCode = '$username'  and  ds.name like '%$disease_search_str%'  and app.appointmentType >0   
			group by  app.patientCode
			order by p.name
	";	
	//echo $sql;die;
...
function getPatientByDrugs($username, $drug_search_str){
	$sql = "
			...
			WHERE app.doctorCode = '$username'  and  dg.drugName like '%$drug_search_str%'  and app.appointmentType >0   
			group by  app.patientCode
			order by p.name
	";
	//echo $sql;die;
...
function getAppointmentByDisease($username, $disease_search_str){
	$sql = "
		...
		left join diagnosis d on d.appointMentID=app.appointmentID
		left join disease ds on d.diseaseID = ds.id
		WHERE app.doctorCode = '$username'  and  ds.name like '%$disease_search_str%'
		group by app.patientCode
...
function getAppointmentByDrugs($username, $drug_search_str){
	$sql = "SELECT app.patientCode,  app.doctorCode, 	app.appointmentID, dg.drugName,  ds.name diseaseName,  pt.typeName, app.date, app.date, app.time, app.status, app.addedBy, p.patientID, p.name, p.age, p.address, p.phone, p.sex, IFNULL(p.name, 0) AS patientState, 
			...
			WHERE  app.doctorCode = '$username' AND   dg.drugName like '%$drug_search_str%'
			group by app.patientCode
			order by app.appointmentID DESC";
...
function getAppointmentByPatientType($username, $type_search_str){
	$sql = "SELECT distinct  app.doctorCode, p.patientCode, pt.typeName, ds.name diseaseName, p.name, p.age,app.date,  p.address, p.phone, p.sex, pt.typeName
			...
			left join diagnosis d on d.appointMentID=app.appointmentID
			left join disease ds on d.diseaseID = ds.id
			WHERE  app.doctorCode = '$username'  AND  typeName like '%$type_search_str%'   and app.appointmentType >0  
			group by  app.patientCode
			order by p.name
		";
	//echo $sql;die;
...
function getAllPatient($username){
	$sql = "SELECT distinct  app.doctorCode, p.patientCode, pt.typeName, app.date, ds.name diseaseName,  p.name, p.age, p.address, p.phone, p.sex, pt.typeName
			...
			left join diagnosis d on d.appointMentID=app.appointmentID
			left join disease ds on d.diseaseID = ds.id		
			WHERE  app.doctorCode = '$username'   and app.appointmentType = 0   
			group by  app.patientCode
			order by p.name ";
	//echo $sql;die;
	$result=mysql_query($sql);
